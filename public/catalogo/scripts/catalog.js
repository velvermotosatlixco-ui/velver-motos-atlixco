...
document.addEventListener('DOMContentLoaded', () => {
    ...
    motos.forEach((moto) => {
        ...
        card.className = "exp-card opacity-0 translate-y-10"; // Clases iniciales para animación GSAP
        card.innerHTML = `
            <div class="h-[280px] overflow-hidden rounded-[24px] mb-6 bg-gray-900">
                <img src="${moto.imagen}" alt="${moto.nombre}" class="w-full h-full object-cover hover:scale-110 transition-transform duration-500">
            </div>
            <h3 class="text-2xl font-bold mb-2 text-white">${moto.nombre}</h3>
            <span class="text-[#3BA0FF] font-semibold text-sm uppercase tracking-wider">${moto.categoria}</span>
